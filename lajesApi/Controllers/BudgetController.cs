            ...
            ) =>
            {
                try
                {
                    ...
                    existingBudget.UpdateBudget(
                        ...
                        await freightRepository.GetFreightById(newBudgetRequest.freightId), // Busca o Freight se necessário
                        null // Não vamos passar as slabs aqui ainda
                    );

                    // Obter as BudgetSlabs existentes
                    var existingSlabs = existingBudget.Slabs.ToList(); // Convertemos para uma lista para manipulação

                    // Armazena os IDs das slabs que já existem
                    var existingSlabIds = existingSlabs.Select(s => s.Id).ToHashSet();

                    foreach (var slabRequest in newBudgetRequest.budgetSlabs)
                    {
